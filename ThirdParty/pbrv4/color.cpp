...
namespace pbrt {
    ...
    {
        return "[ RGBSigmoidPolynomial c0:" + std::to_string(c0) + " c1: " + std::to_string(c1) + " c2: " + std::to_string(c2) + "]";
    }

    // RGBToSpectrumTable Method Definitions
    RGBSigmoidPolynomial RGBToSpectrumTable::operator()(RGB rgb) const
    {
        if (!(rgb[0] >= 0.f && rgb[1] >= 0.f && rgb[2] >= 0.f && rgb[0] <= 1.f &&
            rgb[1] <= 1.f && rgb[2] <= 1.f))
        {
            std::cout << "RGBToSpectrumTable::operator() rgb not valid values\n";
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            auto co = [&](int dx, int dy, int dz) {
                //std::cout << maxc << " " << zi + dz << " " << yi + dy << " " << xi + dx << " " << i << std::endl;
                int index = maxc * 64 * 64 * 64 * 3 + (zi + dz) * 64 * 64 * 3 + (yi + dy) * 64 * 3 + (xi + dx) * 3 + i;
                auto val = coeffs[index];
               // auto val = (*coeffs)[maxc][zi + dz][yi + dy][xi + dx][i];
                return val;
            };
    ...
    */
    inline int UtoInt(std::array<unsigned char, 4> bytes)
    {
    ...
    {
        int sRGBToSpectrumTable_Res = 64;
        float* sRGBToSpectrumTable_Scale = new float[64];
        float* sRGBToSpectrumTable_Data = new float[3 * 64 * 64 * 64 * 3];

        std::fstream binary_file;
        binary_file.open("../rgb2spec/sRGB64binary", std::fstream::in | std::fstream::binary);
        if (binary_file.is_open())
        {
            //read 1 float
            std::array<unsigned char, 4> int_binary;
            binary_file.read((char*)int_binary.data(), sizeof(float));
            int value = UtoInt(int_binary);
            std::cout << value << std::endl;

            //read 64 floats
            std::array<std::array<unsigned char, 4>, res> scales;
            for (int i = 0; i < scales.size(); i++)
            {
                binary_file.read((char*)scales[i].data(), sizeof(float));
                float value;
                std::memcpy(&value, scales[i].data(), sizeof(float));
                std::cout << std::setprecision(10) << value << ", ";
                sRGBToSpectrumTable_Scale[i] = value;
            }
            std::cout << "\n\n";
            //read float[3][res][res][res][3];
            int index = 0;
            for (int x = 0; x < 3; x++)
            {
                for (int y = 0; y < res; y++)
                {
                    for (int z = 0; z < res; z++)
                    {
                        for (int a = 0; a < res; a++)
                        {
                            for (int b = 0; b < 3; b++)
                            {
                                std::array<unsigned char, 4> binary_float;
                                binary_file.read((char*)binary_float.data(), sizeof(float));
                                float value = 0;
                                std::memcpy(&value, binary_float.data(), sizeof(float));
                              //  sRGBToSpectrumTable_Data[b][a][z][y][x] = value;
                                sRGBToSpectrumTable_Data[index] = value;
                                index++;
                            }
                        }
                    }
                }
                std::cout << sRGBToSpectrumTable_Data[index-1] << std::endl;
            }
        }
        else
        {
            std::cout << "couldnt open rgb2spec file\n";
        }
       

        RGBToSpectrumTable::sRGB = new RGBToSpectrumTable(sRGBToSpectrumTable_Scale, sRGBToSpectrumTable_Data);

      //  DCI_P3 = new RGBToSpectrumTable(DCI_P3ToSpectrumTable_Scale, &DCI_P3ToSpectrumTable_Data);
      //  Rec2020 = new RGBToSpectrumTable(REC2020ToSpectrumTable_Scale, &REC2020ToSpectrumTable_Data);
      //  ACES2065_1 = new RGBToSpectrumTable(ACES2065_1ToSpectrumTable_Scale, &ACES2065_1ToSpectrumTable_Data);
    }
    ...
    std::string RGBToSpectrumTable::ToString() const
    {
        ...
        /*else if (this == RGBToSpectrumTable::DCI_P3)
            ...
            id = "(ACES2065_1) ";
        //CHECK(!id.empty());*/
        return "[ RGBToSpectrumTable res: " + res + ' ' + id;
    }
    ...
    void sRGBColorEncoding::FromLinear(std::span<const float> vin,std::span<uint8_t> vout) const
    {
    ...
    */
    ...
    const float SRGBToLinearLUT[256] = {
